...
export default function JoinServerModal({
  ...
  const t = useTranslations("join_server_modal");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      <div className="absolute inset-0 bg-[rgba(5,12,25,0.75)] backdrop-blur-sm" onClick={onClose} />
      
      <div className="relative z-10 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-[24px] border border-[var(--stroke)] bg-[var(--surface)] p-5 sm:p-6 shadow-[0_20px_40px_rgba(6,10,20,0.6)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-slate-400">Serveur</p>
            <h2 className="mt-1 font-display text-xl sm:text-2xl text-white">{t("title")}</h2>
          </div>
          <button
